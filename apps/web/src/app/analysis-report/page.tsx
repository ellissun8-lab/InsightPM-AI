...
function loadReportData(caseName: string) {
  ...
  if (fs.existsSync(mdDir)) {
    const mdFiles = fs
      .readdirSync(mdDir)
      .filter((f: string) => f.endsWith(".overall.analysis.md"));
  ...
  // Build flattened clusters from all segments
  ...
  if (fs.existsSync(segmentsDir)) {
    ...
    for (const f of segFiles) {
      ...
      const segName =
        seg?.segment_name || seg?.name || f.replace(".analysis.json", "");
      if (seg?.issue_clusters) {
        for (const c of seg.issue_clusters) {
          ...
          evidenceTrace.push({
            ...
            count: evCount,
            excerpt:
              c.representative_quote || c.evidence?.[0]?.text || c.summary || "",
  ...
  // Sort clusters by priority then score
...
function loadSegmentData(
  ...
  return loadJson(segPath);
}

function loadSegmentMd(
  caseName: string,
  dataset: string,
  segmentId: string
): string | null {
  // Try analysis-md first, then analysis directory
  const mdPaths = [
    path.join(
      ROOT,
      "runs",
...
export default async function AnalysisReportPage({
  ...
  if (!caseName || !isValidParam(caseName)) {
    caseName = allRuns[0].caseName;
  }
  // Verify case exists
  if (!allRuns.find((r) => r.caseName === caseName)) {
    caseName = allRuns[0].caseName;
  }
  ...
  // Resolve segmentId
  const dataset =
    reportData.summary?.dataset || reportData.summary?.datasetName || "";
  let segmentId = params.segment || "";
  if (!segmentId || !isValidParam(segmentId)) {
    // Default to first segment
    segmentId = reportData.segments[0]?.segmentId || "";
  }
  // Verify segment exists
  if (!reportData.segments.find((s: any) => s.segmentId === segmentId)) {
    segmentId = reportData.segments[0]?.segmentId || "";
  }

  // Load segment-specific data
  const segmentData = segmentId
    ? loadSegmentData(caseName, dataset, segmentId)
    : null;
  const segmentMd = segmentId
    ? loadSegmentMd(caseName, dataset, segmentId)
    : null;

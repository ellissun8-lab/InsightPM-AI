...
const EMPTY_DATASETS: DatasetStats = {
  totalDatasets: 0,
  acceptedCount: 0,
  totalFeedbacks: 0,
};
...
export default function DashboardPage() {
  ...
  const [datasets, setDatasets] = useState<DatasetStats>(EMPTY_DATASETS);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setRuns(nextRuns);
      setDatasets(data.datasets || EMPTY_DATASETS);
      setHasTimeData(hasReliableTimeData(nextRuns));
    } catch {
      setRuns([]);
  ...
  const pendingReports = filteredRuns.filter((run) => {
    const status = getRunDisplayStatus(run as RunLike);
    return status !== "completed";
  }).length;

  const getCaseName = (run: RunRecord) => run.caseName || run.case_name || "未命名分析";
  const getScenario = (run: RunRecord) => run.scenario || run.dataset || "mixed-feedback";
  const getFeedbackCount = (run: RunRecord) => run.feedbackCount ?? run.count ?? 0;
      ...
      <div className="flex-1 ml-[280px] flex flex-col min-h-screen">
        <header className="fixed top-0 right-0 left-[280px] h-16 px-8 flex justify-between items-center bg-surface border-b border-outline-variant z-10">
          <div className="relative w-64">
            ...
            <input
              className="w-full pl-10 pr-4 py-2 bg-surface-bright border border-outline-variant rounded-full font-label-sm text-label-sm text-on-surface focus:outline-none focus:border-primary transition-colors"
              placeholder="搜索洞察..."
              type="text"
            />
          </div>
          ...
          <div className="mb-lg flex justify-between items-end">
            <div>
              ...
              <p className="font-body-md text-body-md text-on-surface-variant">
                {hasTimeData ? "以下是所选时间范围内的最新分析结果。" : "以下是全部分析结果。"}
              </p>
            </div>
            <DashboardToolbar
              timeRange={timeRange}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-md mb-xl">
            <div className="bg-surface-bright border border-outline-variant rounded-xl p-8 shadow-diffused">
              <div className="flex justify-between items-start mb-6">
                ...
                <span className="material-symbols-outlined text-secondary bg-secondary-container rounded-full p-2 text-[20px]">
                  forum
                </span>
              </div>
              <div className="font-display-lg text-display-lg text-primary mb-2">
                {datasets.acceptedCount}
              </div>
            ...
            <div className="bg-surface-bright border border-outline-variant rounded-xl p-8 shadow-diffused">
              <div className="flex justify-between items-start mb-6">
                ...
                <span className="material-symbols-outlined text-primary bg-surface-variant rounded-full p-2 text-[20px]">
                  verified
                </span>
              </div>
              <div className="font-display-lg text-display-lg text-primary mb-2">
                {filteredRuns.filter((run) => getRunDisplayStatus(run as RunLike) === "completed").length}
              </div>
            ...
            <div className="bg-surface-bright border border-outline-variant rounded-xl p-8 shadow-diffused">
              <div className="flex justify-between items-start mb-6">
                ...
                <span className="material-symbols-outlined text-on-surface bg-surface-container rounded-full p-2 text-[20px]">
                  pending_actions
                </span>
              </div>
              <div className="font-display-lg text-display-lg text-primary mb-2">{pendingReports}</div>
              <div className="font-label-sm text-label-sm flex items-center gap-1 text-on-surface-variant">
                包含复核、失败、部分完成和等待中
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-md">
            <div className="lg:col-span-8 bg-surface-bright border border-outline-variant rounded-xl shadow-diffused overflow-hidden">
              <div className="p-6 border-b border-outline-variant flex justify-between items-center bg-surface-bright">
                ...
                <a href="/runs" className="font-label-md text-label-md text-primary hover:text-on-surface-variant transition-colors">
                  查看全部
                </a>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-surface border-b border-outline-variant">
                      <th className="py-4 px-6 font-label-sm text-label-sm text-on-surface-variant font-medium uppercase tracking-wider">
                        案例名称
                      </th>
                      <th className="py-4 px-6 font-label-sm text-label-sm text-on-surface-variant font-medium uppercase tracking-wider">
                        场景
                      </th>
                      <th className="py-4 px-6 font-label-sm text-label-sm text-on-surface-variant font-medium uppercase tracking-wider text-right">
                        反馈数
                      </th>
                      <th className="py-4 px-6 font-label-sm text-label-sm text-on-surface-variant font-medium uppercase tracking-wider text-right">
                        硬性校验
                  ...
                  <tbody className="font-body-md text-body-md text-primary divide-y divide-outline-variant">
                    ...
                    {!loading &&
                      filteredRuns.slice(0, 5).map((run) => {
                        ...
                        return (
                          <tr key={run.id || caseName} className="hover:bg-surface transition-colors group cursor-pointer">
                            <td className="py-4 px-6 font-medium">
                              <a href={`/runs/${encodeURIComponent(caseName)}`} className="text-primary hover:underline">
                                {caseName}
                              </a>
                            </td>
            ...
            <div className="lg:col-span-4 flex flex-col gap-md">
              <div className="bg-[#EFE4CC] border-l-4 border-primary p-6 rounded-r-xl shadow-[0_4px_16px_rgba(23,21,17,0.03)] relative overflow-hidden">
                ...
                <p className="font-body-md text-body-md text-primary leading-relaxed">
                  近期完成运行占比稳定，需复核项主要集中在语义分不足或证据断链的案例。
                </p>
              </div>
